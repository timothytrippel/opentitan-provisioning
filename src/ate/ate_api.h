...
enum {
  ...
  /** Maximum symmetric key size in bytes. This is equivalent to 256 bits. */
...
/**
 * Generate symmetric key parameters.
 */
typedef struct generate_symmetric_key_params {
  /** Symmetric key type.  */
  symmetric_key_type_t type;
  /** Symmetric key size. */
  symmetric_key_size_t size;
  /** Symmetric key diversifier to use in KDF operation. */
  uint8_t diversifier[32];
} generate_symmetric_key_params_t;
 ...
 *
 * The function generates symmetric keys based on the request parameters.
 * The caller should allocate enough memory to store the generated keys and the
 * wrapped seed.
 *
 * @param client A client instance.
 * @param sku The SKU of the product to generate the keys for.
 * @param keys_count The number of keys to generate.
 * @param key_params The parameters for the key generation.
 * @param[out] keys The generated keys.
 * @param[out] seed The wrapped seed.
 * @return The result of the operation.
 */
